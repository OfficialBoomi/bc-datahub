...
Boomi Companion plugin for Boomi DataHub (master data management).
...
`bc-datahub` plugin and `boomi-datahub` skill read credentials from the workspace `.env`. If `bc-integration` plugin or `boomi-integration` is already setup, some of the those Platform details are re-used by this skill/plugin.
